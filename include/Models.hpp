...
enum class Role { Manager, Boss, Specialist, technician };
...
public:
  // Constructor
  Employee(int id, int personnelCode, const std::string &name,
           const std::string &hireDate, Role role, bool isActive,
           std::optional<int> rt)
      : EmployeeId{id}, personnelCode{personnelCode}, Name{name},
        HireDate{hireDate}, role{role}, isActive{isActive}, ReportsTo{rt} {}
  // Default Constructor
  Employee() {}
  ~Employee() {}
};
...
inline std::ostream &operator<<(std::ostream &os, const Employee &emp) {
  ...
  os << "EmployeeId: " << emp.EmployeeId << std::endl
     ...
     << "ReportsTo: "
     << (emp.ReportsTo ? std::to_string(*emp.ReportsTo) : "null");
  return os;
}
...
struct performanceReview {
  ...
  // constructor
  performanceReview(int ReviewId, int EmployeeId, int ReiviewerId,
                    const std::string &reviewDate,
                    std::optional<float> OveralRating, float PunctualityRating,
                    float QualityOfWorkRating, float CommunicatioRating,
                    float TeamworkRating, float ProblemSolvingRating,
                    float CreativityRating, float AdaptabilityRating,
                    float LeadershipRating, float InitiativeRating,
                    const std::string &Comments)
      : ReviewId{ReviewId}, EmployeeId{EmployeeId}, ReviewerId{ReviewerId},
        reviewDate{reviewDate}, OverallRating{OveralRating},
        ...
        LeadershipRating{LeadershipRating}, InitiativeRating{InitiativeRating},
        Comments{Comments} {}
};
